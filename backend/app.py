...
def create_formula(db: Annotated[psycopg.Connection, Depends(connection_factory)], formula: Formula):
    cur = db.cursor(row_factory=dict_row)
    cur.execute("INSERT INTO formulas(name, latex, source) VALUES (%s, %s, %s)", (formula.name, formula.latex, formula.source))
    db.commit()
...
def parse_pdf(db: Annotated[psycopg.Connection, Depends(connection_factory)], file: UploadFile):
    ...
    db.commit()
    return list(map(lambda x: x.strip('$'), matches_large)), list(map(lambda x: x.strip('$'), filter(lambda x: x != '$$', matches_small)))
